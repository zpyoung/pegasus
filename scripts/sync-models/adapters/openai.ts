...
function mapOpenAIModel(m: OpenAIModel): ModelEntry {
  ...
  return {
    ...
    supportsTools: true,
    reasoningCapable: m.id.startsWith('o1') || m.id.startsWith('o3') || m.id.startsWith('o4') || m.id.includes('codex'),
...
export const openaiAdapter: ProviderAdapter = {
  ...
  async fetchModels(): Promise<ModelEntry[]> {
    ...
    if (!data.data || data.data.length === 0) {
      throw new Error('OpenAI API returned empty model list (possible auth failure)');
    }
