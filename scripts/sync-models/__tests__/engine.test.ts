...
const makeModel = (
  overrides: Partial<ModelEntry> & { id: string; provider: string },
): ModelEntry => ({
  name: overrides.id,
  ...overrides,
});
...
const MODELS_B: ModelEntry[] = [makeModel({ id: "model-c", provider: "p2" })];
...
describe("applyOverrides", () => {
  it("merges override fields into matching model", () => {
    ...
    const overrides = {
      "model-a": { name: "Override Name", supportsVision: true },
    };
  ...
  it("leaves models without overrides unchanged", () => {
    const models: ModelEntry[] = [
      makeModel({ id: "model-a", provider: "p1", name: "Original" }),
    ];
...
describe("computeDiff", () => {
  it("identifies added models", () => {
    const diff = computeDiff(
      ...
      [
        makeModel({ id: "old-model", provider: "p1" }),
        makeModel({ id: "new-model", provider: "p1" }),
      ],
      [],
      [],
    );
...
describe("runSync (NFR-003: mixed pass/fail adapters)", () => {
    ...
    return {
      ...
      fetchModels: resolves
        ? vi
            .fn()
            .mockResolvedValue([
              makeModel({ id: `${name}-model-1`, provider: name }),
            ])
  ...
  it("filters ci-tier adapters when ciOnly=true", async () => {
    ...
    await runSync([ciAdapter, localAdapter], {
      ciOnly: true,
      dryRun: true,
      ttlDays: 30,
    });
