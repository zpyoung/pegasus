...
describe('copilotAdapter', () => {
  ...
  it('throws on 401 auth failure', async () => {
    ...
    await expect(copilotAdapter.fetchModels()).rejects.toThrow('Copilot API error: 401');
  });

  it('throws on 429 rate limit', async () => {
    mockFetch(429, { message: 'Too Many Requests' });

    await expect(copilotAdapter.fetchModels()).rejects.toThrow('Copilot API error: 429');
  });
