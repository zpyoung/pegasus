...
test.describe("Multi-Project Dashboard", () => {
  ...
  test("should navigate to overview from dashboard when projects exist", async ({
    page,
  }) => {
    ...
    if (await overviewButton.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      test.info().annotations.push({
        type: "info",
        description:
          "No projects available - skipping overview navigation test",
      });
    }
  });

  test("should display overview view with correct structure", async ({
    page,
  }) => {
  ...
  test("should show loading state and then content or empty state", async ({
    page,
  }) => {
    await page.goto("/overview");
    ...
    const hasProjects =
      (await page.locator('[data-testid^="project-status-card-"]').count()) > 0;
  ...
  test("should have overview link in sidebar footer", async ({ page }) => {
    ...
    if (
      await sidebarOverviewLink.isVisible({ timeout: 3000 }).catch(() => false)
    ) {
      // Should be clickable
