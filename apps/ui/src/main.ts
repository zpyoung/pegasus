...
import {
  startStaticServer,
  stopStaticServer,
} from "./electron/server/static-server";
import {
  startServer,
  waitForServer,
  stopServer,
} from "./electron/server/backend-server";
...
async function handleAppReady(): Promise<void> {
  ...
  if (app.isPackaged) {
    ...
    try {
      ...
      logger.warn(
        "[PRODUCTION] Failed to set userData path:",
    ...
    try {
      ...
      logger.info(
        "[DEVELOPMENT] userData path explicitly set to:",
        userDataPathToUse,
      );
    } catch (error) {
      logger.warn(
  ...
  try {
    ...
    if (skipEmbeddedServer) {
      ...
      logger.info(
        "SKIP_EMBEDDED_SERVER=true, using external server at port",
        state.serverPort,
      );
