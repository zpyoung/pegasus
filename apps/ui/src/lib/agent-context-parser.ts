...
export function formatModelName(
  model: string,
  options?: FormatModelNameOptions,
): string {
  ...
  if (options?.providerId && options?.claudeCompatibleProviders) {
    const provider = options.claudeCompatibleProviders.find(
  ...
  if (model.includes("sonnet-4-6") || model === "claude-sonnet")
    return "Sonnet 4.6";
  ...
  if (model === "cursor-composer-1" || model === "composer-1")
    return "Composer 1";
  if (model.startsWith("cursor-sonnet")) return "Cursor Sonnet";
  if (model.startsWith("cursor-opus")) return "Cursor Opus";
  if (model.startsWith("cursor-gpt"))
    return model.replace("cursor-", "").replace("gpt-", "GPT-");
  ...
  if (model.includes("/") && !model.includes("://")) {
    ...
    if (tierMatch) {
      lastSegment = lastSegment.slice(
        0,
        lastSegment.length - tierMatch[0].length,
      );
    }
    // Clean up the model name for display (remove version tags, capitalize)
    const cleanedName = lastSegment
      .replace(/[-_]/g, " ")
