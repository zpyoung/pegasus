...
export function SidebarNavigation({
  ...
  const { collapsedNavSections, setCollapsedNavSections, toggleNavSection } =
    useAppStore();
  ...
  const visibleSections = navSections.filter((section) => {
    ...
    if (
      !section.label &&
      section.items.some((item) => item.id === "overview")
    ) {
      return true;
    }
    ...
    >
      ...
      {visibleSections.map((section, sectionIdx) => {
        const isCollapsed = section.label
          ? collapsedNavSections[section.label]
          : false;
        const isCollapsible =
          section.collapsible && section.label && sidebarOpen;
        ...
        return (
          <div
            key={sectionIdx}
            className={sectionIdx > 0 && sidebarOpen ? "mt-4" : ""}
          >
            {/* Section Label - clickable if collapsible (expanded sidebar) */}
            {section.label && sidebarOpen && (
              <button
                onClick={() =>
                  isCollapsible && toggleNavSection(section.label!)
                }
                className={cn(
