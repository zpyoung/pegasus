...
export function ExportFeaturesDialog({
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      toast.success(
        `Exported ${featuresToExport.length} feature(s) to ${format.toUpperCase()}`,
      );
      onOpenChange(false);
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "Failed to export features",
      );
    } finally {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent data-testid="export-features-dialog">
        <DialogHeader>
          ...
          <DialogDescription>
            Export {featuresToExport.length} feature(s) to a file for backup or
            ...
            <div className="space-y-2">
              ...
              <div className="max-h-32 overflow-y-auto rounded-md border border-border/50 bg-muted/30 p-2 text-sm">
                {featuresToExport.map((f) => (
                  <div
                    key={f.id}
                    className="py-1 px-2 truncate text-muted-foreground"
                  >
