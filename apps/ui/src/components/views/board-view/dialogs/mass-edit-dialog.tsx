...
function getMixedValues(features: Feature[]): Record<string, boolean> {
  ...
  return {
    ...
    thinkingLevel: !features.every(
      (f) => f.thinkingLevel === first.thinkingLevel,
    ),
    ...
    excludedPipelineSteps: !features.every(
      (f) =>
        JSON.stringify(f.excludedPipelineSteps || []) === firstExcludedSteps,
    ),
  };
}
...
function FieldWrapper({
  ...
  return (
    ...
    >
      ...
      <div className={cn(!willApply && "opacity-50 pointer-events-none")}>
        {children}
      </div>
    </div>
  );
...
export function MassEditDialog({
  ...
  const mixedValues = useMemo(
    () => getMixedValues(selectedFeatures),
    [selectedFeatures],
  );
  ...
  useEffect(() => {
    if (open && selectedFeatures.length > 0) {
      ...
      setProviderId(
        getInitialValue(selectedFeatures, "providerId", undefined) as
          | string
          | undefined,
      );
      setPlanningMode(
      ...
      setRequirePlanApproval(
        getInitialValue(selectedFeatures, "requirePlanApproval", false),
      );
      ...
      const initialBranchName = getInitialValue(
        selectedFeatures,
        "branchName",
        "",
      ) as string;
  ...
  const handleApply = async () => {
    ...
    if (applyState.requirePlanApproval)
      ...
      <DialogContent className="max-w-2xl" data-testid="mass-edit-dialog">
        ...
        <div className="py-4 pr-4 space-y-4 max-h-[60vh] overflow-y-auto">
          ...
          <FieldWrapper
            label="Planning Mode"
            isMixed={
              mixedValues.planningMode || mixedValues.requirePlanApproval
            }
            willApply={
              applyState.planningMode || applyState.requirePlanApproval
            }
            onApplyChange={(apply) =>
              setApplyState((prev) => ({
                ...prev,
          ...
          <FieldWrapper
            ...
            onApplyChange={(apply) =>
              setApplyState((prev) => ({ ...prev, priority: apply }))
            }
          >
            <PrioritySelect
          ...
          <FieldWrapper
            ...
            onApplyChange={(apply) =>
              setApplyState((prev) => ({ ...prev, skipTests: apply }))
            }
          >
            <TestingTabContent
              skipTests={skipTests}
          ...
          <FieldWrapper
            ...
            onApplyChange={(apply) =>
              setApplyState((prev) => ({ ...prev, branchName: apply }))
            }
          >
            <WorkModeSelector
              workMode={workMode}
