...
interface MergeWorktreeDialogProps {
  ...
  onIntegrated: (
    integratedWorktree: WorktreeInfo,
...
export function MergeWorktreeDialog({
  ...
  const [mergeConflict, setMergeConflict] = useState<MergeConflictInfo | null>(
    null,
  );
          ...
          .then((result) => {
            if (result.success && result.result?.branches) {
              ...
              const branches = result.result.branches
                .filter(
                  (b: BranchInfo) => !b.isRemote && b.name !== worktree.branch,
                )
  ...
  const handleMerge = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const parts = [
          `Branch "${worktree.branch}" has been ${mode} into "${targetBranch}"`,
        ];
        if (squash)
          parts.push("All commits were condensed into a single commit.");
        if (deleteWorktreeAndBranch)
          parts.push("The worktree and branch were deleted.");
        toast.success(`Branch ${mode} into ${targetBranch}`, {
              ...
              {worktree.hasChanges && (
                <div className="flex items-start gap-2 p-3 rounded-md bg-yellow-500/10 border border-yellow-500/20">
                  ...
                  <span className="text-yellow-500 text-sm">
                    ...
                    change(s). Please commit or discard them before integrating.
                  </span>
                </div>
              )}
            </div>
        ...
        {squash && (
          <div className="flex items-start gap-2 p-3 rounded-md bg-blue-500/10 border border-blue-500/20">
            ...
            <span className="text-blue-500 text-sm">
              All commits from this branch will be condensed into a single
              commit on the target branch. Individual commit history will not be
              preserved.
            </span>
          </div>
        )}
        ...
        {deleteWorktreeAndBranch && (
          <div className="flex items-start gap-2 p-3 rounded-md bg-orange-500/10 border border-orange-500/20">
            ...
            <span className="text-orange-500 text-sm">
              The worktree and branch will be permanently deleted. Any features
              assigned to this branch will be unassigned.
            </span>
          </div>
        )}
