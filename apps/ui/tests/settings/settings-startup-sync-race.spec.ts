...
test.describe("Settings startup sync race", () => {
  ...
  test.beforeAll(() => {
    ...
    const settings = JSON.parse(originalSettingsJson) as Record<
      string,
      unknown
    >;
    settings.projects = [
  ...
  test("does not overwrite projects when /api/settings/global is temporarily unavailable", async ({
    ...
    const settingsAfterFailures = JSON.parse(
    ...
    const settingsAfterHydration = JSON.parse(
      fs.readFileSync(SETTINGS_PATH, "utf-8"),
    ) as {
      projects?: Array<{ path?: string }>;
    };
