...
const mockUseGenerateIdeationSuggestions = useGenerateIdeationSuggestions as ReturnType<
  typeof vi.fn
>;
...
const mockCategories = [
  { id: 'feature', name: 'Feature', icon: 'lightbulb', description: 'Feature prompts' },
...
describe('PromptCommandPopover', () => {
  ...
  describe('trigger button', () => {
    it('renders the "Generate Ideas" button', () => {
      ...
      expect(screen.getByRole('button', { name: /Generate Ideas/ })).toBeInTheDocument();
    });
  ...
  describe('search filtering', () => {
    ...
    it('shows all prompts again when search is cleared', async () => {
      ...
      await waitFor(() => expect(screen.queryByText('Improve onboarding')).not.toBeInTheDocument());
    ...
    it('calls addGenerationJob with the project path and the selected prompt', async () => {
      ...
      await waitFor(() => {
        ...
        expect(mockAddGenerationJob).toHaveBeenCalledWith(
          TEST_PROJECT,
          expect.objectContaining({ id: 'p1', title: 'Add dark mode', category: 'feature' })
        );
      });
    });
