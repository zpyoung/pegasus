...
describe("MODAL_CONSTANTS", () => {
  ...
  describe("WIDTH_CONSTRAINTS", () => {
    it("should define responsive width constraints", () => {
      expect(MODAL_CONSTANTS.WIDTH_CONSTRAINTS.MOBILE_MAX_CALC).toBe(
        "calc(100% - 2rem)",
      );
  ...
  describe("COMPONENT_HEIGHTS", () => {
    it("should define complete Tailwind class fragments for template interpolation", () => {
      expect(MODAL_CONSTANTS.COMPONENT_HEIGHTS.SMALL_MIN).toBe(
        "sm:min-h-[200px]",
      );
      expect(MODAL_CONSTANTS.COMPONENT_HEIGHTS.SMALL_MAX).toBe(
        "sm:max-h-[60vh]",
      );
    });
  });
