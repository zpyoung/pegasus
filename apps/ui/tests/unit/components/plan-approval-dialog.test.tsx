...
vi.mock('../../../src/components/views/board-view/dialogs/plan-content-viewer', () => ({
...
describe('PlanApprovalDialog — basic rendering', () => {
  ...
  it('shows the plan content via PlanContentViewer', () => {
    ...
    expect(screen.getByTestId('plan-content-viewer')).toHaveTextContent('Step one');
  });
  ...
  it('does not render when open=false', () => {
    ...
    expect(screen.queryByTestId('plan-approval-dialog')).not.toBeInTheDocument();
  });
});
...
describe('PlanApprovalDialog — reject / feedback flow', () => {
  it('shows feedback textarea after clicking "Request Changes"', async () => {
    ...
    expect(screen.getByLabelText(/What changes would you like/i)).toBeInTheDocument();
  });
  ...
  it('"Back" button hides the feedback textarea', async () => {
    ...
    expect(screen.queryByLabelText(/What changes would you like/i)).not.toBeInTheDocument();
  });
  ...
  it('shows "Revise Plan" button when feedback text is entered', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /Request Changes/i }));
    await user.type(screen.getByLabelText(/What changes would you like/i), 'Please add error handling');
  ...
  it('calls onReject with feedback when "Revise Plan" is clicked', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /Request Changes/i }));
    await user.type(screen.getByLabelText(/What changes would you like/i), 'Add more tests');
...
describe('PlanApprovalDialog — isRevising state transitions', () => {
  it('clears feedback state when isRevising transitions to false (revised plan arrives)', async () => {
    ...
    const { rerender, onOpenChange, onApprove, onReject } = renderDialog({ isRevising: false });
    await user.click(screen.getByRole('button', { name: /Request Changes/i }));
    await user.type(screen.getByLabelText(/What changes would you like/i), 'Needs more detail');
    ...
    const REVISED_CONTENT = '## Revised Plan\n\n1. Step one (revised)\n2. Step two (revised)';
    rerender(
      <PlanApprovalDialog
        open={true}
        onOpenChange={onOpenChange}
...
describe('PlanApprovalDialog — state reset on open/content change', () => {
  ...
  it('resets the edited plan when planContent changes while open', async () => {
    ...
    expect(screen.queryByPlaceholderText(/Enter plan content/i)).not.toBeInTheDocument();
    expect(screen.getByTestId('plan-content-viewer')).toHaveTextContent('Task A');
  });
});
