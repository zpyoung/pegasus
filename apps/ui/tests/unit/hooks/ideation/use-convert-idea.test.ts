...
describe("useConvertIdea", () => {
  ...
  it("calls convertToFeature with correct projectPath and ideaId", async () => {
    ...
    const { result } = renderHook(() => useConvertIdea(TEST_PROJECT), {
      wrapper,
    });
    ...
    await waitFor(() =>
      expect(mockToast.error).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          description: expect.stringContaining("status must be 'ready'"),
        }),
      ),
    );
  });
  ...
  it("invalidates ideas and features query keys on success", async () => {
    ...
    await waitFor(() => {
      ...
      const hasIdeasKey = calls.some(
        (c) => c.includes("ideation") && c.includes("ideas"),
      );
  ...
  it("has retry: 0 — does not retry on network failure", async () => {
    ...
    mockIdeationAPI.convertToFeature.mockRejectedValue(
