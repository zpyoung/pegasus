  ...
  describe("race condition scenario simulation", () => {
    it("should track recently completed features until cache refresh clears them", () => {
      ...
      expect(
        useAppStore
          .getState()
          .recentlyCompletedFeatures.has("feature-completed"),
      ).toBe(true);
    ...
    it("should handle multiple features completing simultaneously", () => {
      ...
      expect(
        useAppStore.getState().recentlyCompletedFeatures.has("feature-1"),
      ).toBe(true);
      expect(
        useAppStore.getState().recentlyCompletedFeatures.has("feature-2"),
      ).toBe(true);
      expect(
        useAppStore.getState().recentlyCompletedFeatures.has("feature-3"),
      ).toBe(true);
