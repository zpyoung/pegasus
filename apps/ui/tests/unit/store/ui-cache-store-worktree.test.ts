...
describe("ui-cache-store worktree state hardening", () => {
  ...
  it("syncUICache persists only structurally valid worktree entries", () => {
    syncUICache({
      currentWorktreeByProject: {
        ...
        "/valid-feature": {
          path: "/valid/.worktrees/feature-a",
          branch: "feature/a",
        },
        ...
        "/invalid-shape": { path: "/x" } as unknown as {
          path: string;
          branch: string;
        },
      },
  ...
  it("restoreFromUICache sanitizes worktree map and restores resolved project context", () => {
    useAppStore.setState({
      projects: [
        { id: "project-1", name: "Project One", path: "/project-1" },
      ] as unknown[],
    });
    ...
    const restoredState = appStoreSetState.mock.calls[0][0] as Record<
      string,
      unknown
    >;
