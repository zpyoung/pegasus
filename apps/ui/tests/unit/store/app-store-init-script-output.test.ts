...
describe('init script output store', () => {
  ...
  it('preserves blank lines, tabs, and stream type metadata', () => {
    const { setInitScriptState, appendInitScriptOutput } = useAppStore.getState();
    ...
    const state = useAppStore.getState().getInitScriptState(PROJECT_PATH, BRANCH);
  ...
  it('limits retained output to the most recent chunks', () => {
    ...
    const output = useAppStore.getState().getInitScriptState(PROJECT_PATH, BRANCH)?.output ?? [];
