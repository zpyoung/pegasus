...
test.describe("Projects Overview Dashboard", () => {
  ...
  test("should navigate to overview from sidebar and display overview UI", async ({
    page,
  }) => {
    ...
    await expect(page.getByText("Pegasus Dashboard")).toBeVisible({
      timeout: 5000,
    });
    ...
    await expect(
      overviewHeader.getByRole("button", { name: /Open Project/i }),
    ).toBeVisible();
    await expect(
      overviewHeader.getByRole("button", { name: /New Project/i }),
    ).toBeVisible();
  });
  ...
  test("should display project status cards", async ({ page }) => {
    ...
    const projectCard = page.locator(
      '[data-testid="project-status-card-test-project-1"]',
    );
  ...
  test("should navigate to board when clicking on a project card", async ({
    page,
  }) => {
    overviewMock = makeOverviewResponse({
      projects: [
  ...
  test("should display empty state when no projects exist", async ({
    ...
    await expect(
      page.getByText("Create or open a project to get started"),
    ).toBeVisible();
  });
  ...
  test("should show error state when API fails", async ({ page }) => {
    ...
    await expect(page.getByText("Failed to load overview")).toBeVisible({
      timeout: 10000,
    });
