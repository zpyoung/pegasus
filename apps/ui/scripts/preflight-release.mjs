...
if (!token) {
  console.error(
    "❌ GH_TOKEN / GITHUB_TOKEN not set — cannot preflight release.",
  );
  process.exit(1);
}
...
if (!publishCfg || publishCfg.provider !== "github") {
  console.error(
    "❌ Expected a github publish config in package.json build.publish",
  );
  process.exit(1);
}
...
console.log(
  `🛫 Preflight: checking ${owner}/${repo} for existing release ${tag}...`,
);
...
if (!res.ok) {
  console.error(
    `❌ Unexpected GitHub API response: ${res.status} ${await res.text()}`,
  );
  process.exit(1);
}
...
if (release.draft) {
  console.log(
    `⚠️  Draft release for ${tag} exists — build will upload to it. Proceeding.`,
  );
  process.exit(0);
}
