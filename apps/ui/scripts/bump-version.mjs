...
function bumpVersion(packageJsonPath, packageName) {
  try {
    ...
    if (versionParts.length !== 3) {
      console.error(
        `Error: Invalid version format in ${packageName}: ${oldVersion}`,
      );
...
try {
  ...
  if (uiNewVersion !== serverNewVersion) {
    console.error(
      `Error: Version mismatch! UI: ${uiNewVersion}, Server: ${serverNewVersion}`,
    );
    process.exit(1);
  }
  ...
  console.log(
    `📦 Updated @pegasus/server: ${serverOldVersion} -> ${serverNewVersion}`,
  );
