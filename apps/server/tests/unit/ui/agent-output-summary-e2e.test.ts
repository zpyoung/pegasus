 ...
 */
function getFirstNonEmptySummary(
...
describe("getFirstNonEmptySummary", () => {
  it("should return the first non-empty string", () => {
    expect(getFirstNonEmptySummary(null, undefined, "first", "second")).toBe(
      "first",
    );
  });
  ...
  it("should accept strings with content surrounded by whitespace", () => {
    expect(getFirstNonEmptySummary("  content with spaces  ")).toBe(
      "  content with spaces  ",
    );
  });
});
...
describe("Agent Output Summary E2E Flow", () => {
  ...
  beforeEach(() => {
    ...
    (getFeatureDir as Mock).mockReturnValue(
      "/project/.pegasus/features/e2e-feature-1",
    );
  });
  ...
  describe("complete pipeline flow: server accumulation → UI display", () => {
    it("should maintain complete summary across all pipeline steps", async () => {
      ...
      expect(phases2.get("code review")).toContain(
        "Approved with minor suggestions",
      );
      ...
      const implIndex = summaryLines.findIndex((l) =>
        l.includes("### Implementation"),
      );
      const reviewIndex = summaryLines.findIndex((l) =>
        l.includes("### Code Review"),
      );
      const testIndex = summaryLines.findIndex((l) =>
        l.includes("### Testing"),
      );
...
`;
      ...
      const displaySummary = getFirstNonEmptySummary(
        featureSummary,
        extractSummary(rawOutput),
      );
