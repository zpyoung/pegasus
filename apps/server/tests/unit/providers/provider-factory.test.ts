  ...
  describe("getProviderForModel", () => {
    describe("Claude models (claude-* prefix)", () => {
      ...
      it("should return ClaudeProvider for claude-sonnet-4-6", () => {
        const provider =
          ProviderFactory.getProviderForModel("claude-sonnet-4-6");
        expect(provider).toBeInstanceOf(ClaudeProvider);
      });
    ...
    describe("Cursor models (cursor-* prefix)", () => {
      ...
      it("should return CursorProvider for cursor-sonnet-4.5", () => {
        const provider =
          ProviderFactory.getProviderForModel("cursor-sonnet-4.5");
        expect(provider).toBeInstanceOf(CursorProvider);
      });
    ...
    describe("Unknown models", () => {
      ...
      it("should default to ClaudeProvider for completely unknown prefixes", () => {
        const provider =
          ProviderFactory.getProviderForModel("random-xyz-model");
        expect(provider).toBeInstanceOf(ClaudeProvider);
      });
    });
  ...
  describe("getAllProviders", () => {
    ...
    it("should include CopilotProvider", () => {
      ...
      const hasCopilotProvider = providers.some(
        (p) => p instanceof CopilotProvider,
      );
    ...
    it("should include GeminiProvider", () => {
      const providers = ProviderFactory.getAllProviders();
      const hasGeminiProvider = providers.some(
        (p) => p instanceof GeminiProvider,
      );
    ...
    it("should include CursorProvider", () => {
      const providers = ProviderFactory.getAllProviders();
      const hasCursorProvider = providers.some(
        (p) => p instanceof CursorProvider,
      );
