...
import {
  CopilotProvider,
  CopilotErrorCode,
} from "@/providers/copilot-provider.js";
...
describe("copilot-provider.ts", () => {
  ...
  describe("executeQuery resume behavior", () => {
    it("uses resumeSession when sdkSessionId is provided", async () => {
      ...
      expect(results.some((msg) => msg.session_id === "resumed-session")).toBe(
        true,
      );
    });
    ...
    it("falls back to createSession when resumeSession fails", async () => {
      ...
      createSessionMock.mockResolvedValueOnce(
        createMockSession("fresh-session"),
      );
      ...
      expect(results.some((msg) => msg.session_id === "fresh-session")).toBe(
        true,
      );
    });
  });
