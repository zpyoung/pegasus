...
describe("running-agents routes", () => {
  ...
  describe("GET / (index handler)", () => {
    ...
    it("should handle non-Error exceptions", async () => {
      // Arrange
      vi.mocked(mockAutoModeService.getRunningAgents!).mockRejectedValue(
        "String error",
      );
    ...
    it("should include agents from different projects", async () => {
      ...
      expect(response.runningAgents[0].projectPath).toBe(
        "/workspace/project-alpha",
      );
      expect(response.runningAgents[1].projectPath).toBe(
