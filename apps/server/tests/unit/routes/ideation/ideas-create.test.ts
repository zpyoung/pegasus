...
describe('POST /ideas/create', () => {
  ...
  it('returns 400 when idea body is missing', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.json).toHaveBeenCalledWith({ success: false, error: 'idea is required' });
    expect(mockIdeationService.createIdea).not.toHaveBeenCalled();
  });
  ...
  it('returns 400 when idea.title is missing (FR-001 guard)', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.json).toHaveBeenCalledWith({ success: false, error: 'idea must have a title' });
    expect(mockIdeationService.createIdea).not.toHaveBeenCalled();
  });
  ...
  it('accepts title-only payload and returns 200 (FR-001 quick-add)', async () => {
    ...
    vi.mocked(mockIdeationService.createIdea!).mockResolvedValue(created as any);

    req.body = { projectPath: TEST_PROJECT_PATH, idea: { title: 'Quick idea' } };
    const handler = createIdeasCreateHandler(
      mockEvents as EventEmitter,
      mockIdeationService as IdeationService
    );
    ...
    expect(mockIdeationService.createIdea).toHaveBeenCalledWith(TEST_PROJECT_PATH, {
      title: 'Quick idea',
    });
  ...
  it('accepts title + description + category payload (NFR-004 backward compat)', async () => {
    ...
    req.body = {
      projectPath: TEST_PROJECT_PATH,
      idea: { title: 'Full idea', description: 'A description', category: 'bug' },
    };
    const handler = createIdeasCreateHandler(
      mockEvents as EventEmitter,
      mockIdeationService as IdeationService
  ...
  it('returns 500 when the service throws an unexpected error', async () => {
    vi.mocked(mockIdeationService.createIdea!).mockRejectedValue(new Error('Disk full'));
    ...
    expect(res.json).toHaveBeenCalledWith({ success: false, error: 'Disk full' });
  });
});
