...
describe('POST /add-suggestion', () => {
  ...
  it('returns 400 when projectPath is missing', async () => {
    req.body = { suggestion: { id: 's1', title: 'T', description: 'D', rationale: 'R', category: 'feature' } };
  ...
  it('returns 400 when suggestion is missing', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.json).toHaveBeenCalledWith({ success: false, error: 'suggestion is required' });
    expect(mockIdeationService.createIdea).not.toHaveBeenCalled();
  });
  ...
  it('returns 400 when suggestion.title is missing', async () => {
    req.body = {
      ...
      suggestion: { id: 's1', description: 'D', rationale: 'R', category: 'feature' },
    };
    const handler = createAddSuggestionHandler(
      mockIdeationService as IdeationService,
      mockFeatureLoader as FeatureLoader
    ...
    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.json).toHaveBeenCalledWith({ success: false, error: 'suggestion.title is required' });
    expect(mockIdeationService.createIdea).not.toHaveBeenCalled();
  });
  ...
  it('appends rationale to description when rationale is provided', async () => {
    const idea = { id: 'idea-2', title: 'T', description: '', category: 'feature', status: 'raw', impact: 'medium', effort: 'medium', createdAt: '', updatedAt: '' };
    vi.mocked(mockIdeationService.createIdea!).mockResolvedValue(idea as any);
    ...
    const calledWith = vi.mocked(mockIdeationService.createIdea!).mock.calls[0][1];
  ...
  it('returns 500 when createIdea throws an unexpected error', async () => {
    vi.mocked(mockIdeationService.createIdea!).mockRejectedValue(new Error('Write failed'));

    req.body = {
      projectPath: TEST_PROJECT_PATH,
      suggestion: { id: 's3', title: 'T', description: 'D', rationale: '', category: 'feature' },
    };
    const handler = createAddSuggestionHandler(
      mockIdeationService as IdeationService,
      mockFeatureLoader as FeatureLoader
