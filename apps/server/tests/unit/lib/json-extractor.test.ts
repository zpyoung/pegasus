...
import {
  extractJson,
  extractJsonWithKey,
  extractJsonWithArray,
} from "@/lib/json-extractor.js";
...
That's all!`;
        ...
        expect(mockLogger.debug).toHaveBeenCalledWith(
          "Extracting JSON from ```json code block",
        );
      });
...
\`\`\``;
        ...
        expect(mockLogger.debug).toHaveBeenCalledWith(
          "Extracting JSON from ``` code block",
        );
      });
...
\`\`\``;
        ...
        const result = extractJson<number[]>(responseText, {
          logger: mockLogger,
        });
...
Here is the JSON: {"actual": "json"}`;
    ...
    describe("Strategy 4: Find any JSON by brace matching", () => {
      it("should extract JSON by matching braces", () => {
        ...
        expect(mockLogger.debug).toHaveBeenCalledWith(
          "Extracting JSON by brace matching",
        );
      });
  ...
  describe("extractJsonWithKey", () => {
    it("should extract JSON with specified required key", () => {
      ...
      const result = extractJsonWithKey(responseText, "suggestions", {
        logger: mockLogger,
      });
  ...
  describe("extractJsonWithArray", () => {
    it("should extract JSON with array at specified key", () => {
      ...
      const result = extractJsonWithArray(responseText, "features", {
        logger: mockLogger,
      });
