...
describe('pegasus-paths.ts', () => {
  ...
  describe('getPegasusDir', () => {
    it('should return path to .pegasus directory', () => {
      expect(getPegasusDir(projectPath)).toBe(path.join(projectPath, '.pegasus'));
    });

    it('should handle paths with trailing slashes', () => {
      const pathWithSlash = path.join('/test', 'project') + path.sep;
      expect(getPegasusDir(pathWithSlash)).toBe(path.join(pathWithSlash, '.pegasus'));
    });
  });

  describe('getFeaturesDir', () => {
    it('should return path to features directory', () => {
      expect(getFeaturesDir(projectPath)).toBe(path.join(projectPath, '.pegasus', 'features'));
    });
  });
  ...
  describe('getBoardDir', () => {
    it('should return path to board directory', () => {
      expect(getBoardDir(projectPath)).toBe(path.join(projectPath, '.pegasus', 'board'));
    });
  });

  describe('getImagesDir', () => {
    it('should return path to images directory', () => {
      expect(getImagesDir(projectPath)).toBe(path.join(projectPath, '.pegasus', 'images'));
    });
  });

  describe('getWorktreesDir', () => {
    it('should return path to worktrees directory', () => {
      expect(getWorktreesDir(projectPath)).toBe(path.join(projectPath, '.pegasus', 'worktrees'));
    });
  });
