...
<project_specification>
  ...
  <implemented_features>
    ...
    it("should extract file_locations when present", () => {
      ...
      expect(features[1].file_locations).toEqual([
        "src/feature-two.ts",
        "src/utils/helper.ts",
      ]);
    });
...
<project_specification>
  ...
  <core_capabilities>
    <capability>Testing</capability>
      ...
      const result = updateImplementedFeaturesSection(
        specWithoutSection,
        newFeatures,
      );
...
<project_specification>
  ...
  describe("custom logger", () => {
    ...
    it("should use custom logger for extractImplementedFeatures", () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        "Extracted 1 implemented features",
      );
    });
    ...
    it("should log when no implemented_features section found", () => {
      ...
      extractImplementedFeatures(xml, { logger: mockLogger });
      expect(mockLogger.debug).toHaveBeenCalledWith(
        "No implemented_features section found",
      );
    });
  ...
  describe("edge cases", () => {
    ...
    describe("extractXmlSection edge cases", () => {
      ...
      it("should handle tags with attributes", () => {
        const xml =
          '<root><section id="1" class="test">content</section></root>';
...
<project_specification>
  ...
  <core_capabilities>
    <capability>User management</capability>
      ...
      expect(extractImplementedFeatureNames(spec)).toEqual([
        "User Authentication",
        "User Profile",
      ]);
  ...
  <core_capabilities></core_capabilities>
      ...
      expect(features[0].description).toBe(
        'Supports <query> syntax with "quoted" terms',
      );
