  ...
  describe("loadPipeline", () => {
    ...
    it("should return all expected config properties", async () => {
      ...
      expect(config.description).toBe(
        "Plan, implement, and test a new feature",
      );
    ...
    it("should throw on invalid YAML content (fails schema validation)", async () => {
      ...
      await expect(loadPipeline(PROJECT_PATH, "bad")).rejects.toThrow(
        /failed validation/,
      );
    });
    ...
    it("should load a pipeline with declared inputs", async () => {
      vi.mocked(secureFs.readFile).mockResolvedValue(
        PIPELINE_WITH_INPUTS_YAML as any,
      );
    ...
    it("should throw when YAML parses to null", async () => {
      ...
      await expect(loadPipeline(PROJECT_PATH, "empty")).rejects.toThrow(
        /failed validation/,
      );
    });
...
`;
      ...
      await expect(loadPipeline(PROJECT_PATH, "dup")).rejects.toThrow(
        /failed validation/,
      );
    });
  });
  ...
  describe("loadAndCompilePipeline", () => {
    ...
    it("should propagate load errors", async () => {
      ...
      await expect(
        loadAndCompilePipeline(PROJECT_PATH, "missing"),
      ).rejects.toThrow(/Pipeline "missing" not found/);
    });
  });
  ...
  describe("validatePipeline", () => {
    ...
    it("should reject a pipeline with duplicate stage IDs", () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.some((e) => e.message.includes("unique"))).toBe(
        true,
      );
    });
    ...
    it("should reject extra unknown fields in a stage (strict mode)", () => {
      const result = validatePipeline({
        ...
        stages: [
          { id: "step", name: "Step", prompt: "Do thing", unknown_flag: true },
        ],
      });
    ...
    it("should include path in error for nested validation issues", () => {
      ...
      const hasNestedPath = result.errors.some(
        (e) =>
          e.path.includes("stages") ||
          e.path.includes("claude_flags") ||
          e.path.includes("max_turns"),
      );
    ...
    it("should accept a pipeline with a valid inputs section", () => {
        ...
        stages: [
          {
            id: "step",
            name: "Step",
            prompt: "Build {{inputs.target_module}}",
          },
        ],
        inputs: {
          target_module: {
      ...
      expect(result.config!.inputs!.target_module.description).toBe(
        "Module to build",
      );
    });
  ...
  describe("extractTemplateVariables", () => {
    it("should extract simple variables", () => {
      const vars = extractTemplateVariables(
        "Hello {{task.description}} in {{project.language}}",
      );
    ...
    it("should deduplicate variables", () => {
      const vars = extractTemplateVariables(
        "{{task.description}} and {{task.description}}",
      );
    ...
    it("should extract variables with underscores", () => {
      const vars = extractTemplateVariables(
        "{{project.test_command}} and {{project.lint_command}}",
      );
  ...
  describe("compileStage", () => {
    ...
    it("should detect multiple missing variables", () => {
      const stage = makeResolvedStage({
        prompt:
          "Test {{project.test_command}} and lint {{project.lint_command}} for {{project.language}}",
      });
    ...
    it("should resolve multiple project properties", () => {
      const stage = makeResolvedStage({
        prompt:
          "Language: {{project.language}}, Test: {{project.test_command}}, Lint: {{project.lint_command}}",
      });
      ...
      expect(result.stage.prompt).toBe(
        "Language: Python, Test: pytest, Lint: ruff check",
      );
      expect(result.hasMissingVariables).toBe(false);
    });
    ...
    it("should resolve task.title when provided", () => {
      ...
      expect(result.stage.prompt).toBe(
        "Title: Login Feature, Desc: Add a login page",
      );
      expect(result.hasMissingVariables).toBe(false);
    });
    ...
    it("should handle mixed present and missing variables", () => {
      const stage = makeResolvedStage({
        prompt:
          "Build {{task.description}} in {{project.language}} using {{project.framework}}",
      });
    ...
    it("should resolve all context namespaces in one prompt", () => {
      const stage = makeResolvedStage({
        prompt:
          "Task: {{task.description}}, Lang: {{project.language}}, Input: {{inputs.target}}, Prev: {{previous_context}}",
      });
  ...
  describe("discoverPipelines", () => {
    ...
    it("should discover user-level pipelines only", async () => {
      ...
      vi.mocked(systemPaths.systemPathReadFile).mockResolvedValue(
        USER_PIPELINE_YAML,
      );
    ...
    it("should override user pipeline with project pipeline while keeping non-overlapping user pipelines", async () => {
      ...
      vi.mocked(systemPaths.systemPathReadFile).mockImplementation(
    ...
    it("should handle directory read errors gracefully (non-ENOENT)", async () => {
      ...
      vi.mocked(systemPaths.systemPathReaddir).mockRejectedValue(
        new Error("EACCES"),
      );
    ...
    it("should correctly count stages in discovered pipelines", async () => {
      ...
      vi.mocked(secureFs.readFile).mockImplementation(
        async (filePath: string) => {
          if ((filePath as string).includes("feature.yaml"))
            return VALID_PIPELINE_YAML as any;
          if ((filePath as string).includes("bug-fix.yaml"))
            return BUG_FIX_PIPELINE_YAML as any;
          throw new Error("Unexpected file");
        },
      );
    ...
    it("should skip invalid pipeline but still include valid ones in same directory", async () => {
      ...
      vi.mocked(secureFs.readFile).mockImplementation(
        async (filePath: string) => {
          if ((filePath as string).includes("feature.yaml"))
            return VALID_PIPELINE_YAML as any;
          if ((filePath as string).includes("broken.yaml"))
            return "name: incomplete" as any;
          throw new Error("Unexpected file");
        },
      );
    ...
    it("should include full config in discovered pipeline", async () => {
      ...
      expect(result[0].config.name).toBe("Feature");
      expect(result[0].config.description).toBe(
        "Plan, implement, and test a new feature",
      );
