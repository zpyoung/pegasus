...
describe("FeatureStateManager", () => {
  ...
  beforeEach(() => {
    ...
    (getFeatureDir as Mock).mockReturnValue(
      "/project/.pegasus/features/feature-123",
    );
  ...
  describe("updateFeatureStatus", () => {
    it("should update feature status and persist to disk", async () => {
      ...
      await manager.updateFeatureStatus(
        "/project",
        "feature-123",
        "in_progress",
      );
    ...
    it("should not fail if sync to app_spec fails", async () => {
      ...
      (mockFeatureLoader.syncFeatureToAppSpec as Mock).mockRejectedValue(
  ...
  describe("markFeatureInterrupted", () => {
    it("should mark feature as interrupted", async () => {
      ...
      await manager.markFeatureInterrupted(
        "/project",
        "feature-123",
        "server shutdown",
      );
  ...
  describe("resetStuckFeatures", () => {
    ...
    it("should handle features directory not existing", async () => {
      ...
      // Should not throw
      await expect(
        manager.resetStuckFeatures("/project"),
      ).resolves.not.toThrow();
    });
  ...
  describe("updateFeaturePlanSpec", () => {
    it("should update planSpec with partial updates", async () => {
      ...
      await manager.updateFeaturePlanSpec("/project", "feature-123", {
        status: "approved",
      });
  ...
  describe("saveFeatureSummary", () => {
    ...
    it("should normalize existing non-phase summary before appending pipeline step summary", async () => {
      const existingSummary =
        "Implemented authentication and settings management.";
      (pipelineService.getStep as Mock).mockResolvedValue({
        name: "Code Review",
        id: "step1",
      });
    ...
    it("should replace existing step summary if called again for the same step", async () => {
      ...
      expect(
        savedFeature.summary.match(
          new RegExp(PIPELINE_SUMMARY_HEADER_PREFIX + "Code Review", "g"),
        )?.length,
      ).toBe(1);
      expect(
        savedFeature.summary.match(
          new RegExp(PIPELINE_SUMMARY_SEPARATOR.trim(), "g"),
        )?.length,
      ).toBe(1);
    });
    ...
    it("should handle pipelineService.getStepIdFromStatus throwing an error gracefully", async () => {
      ...
      expect(savedFeature.summary).toBe(
        `${PIPELINE_SUMMARY_HEADER_PREFIX}My Step\n\nStep output`,
      );
    });

    it("should handle pipelineService.getStep throwing an error gracefully", async () => {
      (pipelineService.getStep as Mock).mockRejectedValue(
  ...
  describe("updateTaskStatus", () => {
    ...
    it("should update task status and summary and emit event", async () => {
      ...
      const savedFeature = (atomicWriteJson as Mock).mock
        .calls[0][1] as Feature;
      expect(savedFeature.planSpec?.tasks?.[0].status).toBe("completed");
      expect(savedFeature.planSpec?.tasks?.[0].summary).toBe(
        "Task finished successfully",
      );
