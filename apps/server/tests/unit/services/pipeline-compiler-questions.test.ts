...
// Import after mocks
import { stageConfigSchema, compilePipeline, compileStage } from '@/services/pipeline-compiler.js';
...
describe('stageConfigSchema — question fields', () => {
  ...
  it('should accept a stage with question_meta (single-select with options)', () => {
    ...
    if (result.success) {
      ...
      expect(result.data.question_meta?.options).toEqual(['Option A', 'Option B', 'Option C']);
    }
  });
...
describe('compilePipeline — question field pass-through', () => {
  ...
  it('should include question_meta in ResolvedStage when present', () => {
    ...
    expect(stages[0].question_meta).toEqual({ type: 'single-select', options: ['A', 'B'] });
  });
...
describe('compileStage — stages context template variables', () => {
  it('should resolve {{stages.plan.question_response}} when provided', () => {
    const stage = createBaseStage({
      prompt: 'Given the approach "{{stages.plan.question_response}}", implement the feature.',
    });
    ...
    expect(result.stage.prompt).not.toContain('{{stages.plan.question_response}}');
  });

  it('should resolve {{stages.plan.question_responses}} when provided', () => {
    const stage = createBaseStage({
      prompt: 'Context: {{stages.plan.question_responses}}\n\nImplement the feature.',
    });
