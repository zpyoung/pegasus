...
describe("DevServerService Persistence & Sync", () => {
  ...
  beforeEach(async () => {
    ...
    testDataDir = path.join(
      os.tmpdir(),
      `dev-server-persistence-test-${Date.now()}`,
    );
    worktreeDir = path.join(
      os.tmpdir(),
      `dev-server-worktree-test-${Date.now()}`,
    );
  ...
  it("should load state from dev-servers.json on initialize", async () => {
    ...
    await fs.writeFile(
      path.join(testDataDir, "dev-servers.json"),
      JSON.stringify(persistedInfo),
    );
  ...
  it("should update persisted state when URL is detected", async () => {
    ...
    mockProcess.stdout.emit(
      "data",
      Buffer.from("Local: http://localhost:5555/\n"),
    );
