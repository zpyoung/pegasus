...
import {
  WorktreeResolver,
  type WorktreeInfo,
} from "@/services/worktree-resolver.js";
...
`;
    ...
    it("should find worktree by branch name", async () => {
      ...
      expect(result).toBe(
        normalizePath("/Users/dev/project/.worktrees/feature-x"),
      );
    });
    ...
    it("should return null when branch not found", async () => {
      ...
      const path = await resolver.findWorktreeForBranch(
        "/Users/dev/project",
        "non-existent",
      );
...
`;
      ...
      expect(result).toBe(
        normalizePath("/Users/dev/project/.worktrees/feature-relative"),
      );
    });
...
`;
      ...
      expect(worktrees[1].path).toBe(
        normalizePath("/Users/dev/project/.worktrees/relative-wt"),
      );
    });
