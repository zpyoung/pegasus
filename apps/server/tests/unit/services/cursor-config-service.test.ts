...
describe("cursor-config-service.ts", () => {
  ...
  describe("readProjectConfig", () => {
    ...
    it("should throw on other errors", async () => {
      ...
      await expect(readProjectConfig(testProjectPath)).rejects.toThrow(
        "Read error",
      );
    });
  });
  ...
  describe("deleteProjectConfig", () => {
    ...
    it("should throw on other errors", async () => {
      ...
      await expect(deleteProjectConfig(testProjectPath)).rejects.toThrow(
        "Permission denied",
      );
    });
  });
  ...
  describe("getEffectivePermissions", () => {
    it("should return project permissions if available", async () => {
      ...
      vi.mocked(fs.readFile).mockResolvedValueOnce(
        JSON.stringify({ permissions: projectPerms }),
      );
    ...
    it("should return global permissions if no project path provided", async () => {
      ...
      vi.mocked(fs.readFile).mockResolvedValue(
        JSON.stringify({ permissions: globalPerms }),
      );
  ...
  describe("applyProfileToProject", () => {
    ...
    it("should throw on unknown profile", async () => {
      await expect(
        applyProfileToProject(testProjectPath, "unknown" as any),
      ).rejects.toThrow("Unknown permission profile: unknown");
    });
  });
