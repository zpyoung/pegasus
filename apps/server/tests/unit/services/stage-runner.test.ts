...
import type { Feature, ResolvedStage, PipelineExecutionState } from '@pegasus/types';
...
describe('StageRunner', () => {
  ...
  describe('fresh execution', () => {
    ...
    it('should emit pipeline_step_started and pipeline_step_complete events for each stage', async () => {
      ...
      const emitCalls = (eventBus.emitAutoModeEvent as ReturnType<typeof vi.fn>).mock.calls;
  ...
  describe('per-stage output persistence', () => {
    ...
    it('should create stage-outputs directory before writing', async () => {
      ...
      const stageOutputDirCreations = mkdirCalls.filter(
        ([dirPath]: [string]) => dirPath.includes('stage-outputs')
      );
    ...
    it('should persist pipeline state after each stage completes', async () => {
      ...
      const lastStateJson = JSON.parse(stateWrites[2][1] as string) as PipelineExecutionState;
    ...
    it('should include accumulated context snapshot in pipeline state', async () => {
      ...
      const firstState = JSON.parse(stateWrites[0][1] as string) as PipelineExecutionState;
      ...
      const secondState = JSON.parse(stateWrites[1][1] as string) as PipelineExecutionState;
  ...
  describe('resumption', () => {
    it('should skip completed stages and resume from next stage', async () => {
      ...
      const existingState = createPipelineState(['plan'], stages, PIPELINE_NAME, [
        'Plan output context',
      ]);
  ...
  describe('abort handling', () => {
    ...
    it('should include stagesSkipped in abort result when resuming', async () => {
      ...
      const existingState = createPipelineState(['plan'], stages, PIPELINE_NAME, ['Plan output']);
  ...
  describe('error handling', () => {
    it('should return failure result when a stage throws', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.aborted).toBe(false);
      expect(result.error).toBe('Stage "implement" failed: Implementation failed');
  ...
  describe('saveStageOutput', () => {
    it('should create directory and write output file', async () => {
      ...
      await runner.saveStageOutput(PROJECT_PATH, FEATURE_ID, 'plan', 'Plan output');
  ...
  describe('buildStagePrompt (via run)', () => {
    it('should include stage name and pipeline metadata', async () => {
      ...
      expect(prompt).toContain('This is an automated pipeline stage execution.');
    });
    ...
    it('should include feature context', async () => {
      ...
      const config = createMockConfig({
        stages: [createMockStage('plan', 'Planning')],
        feature: createMockFeature({ title: 'Dark Mode', description: 'Add dark mode' }),
      });
      await runner.run(config);
  ...
  describe('sequential execution ordering', () => {
    it('should execute stages in array order', async () => {
      ...
      runAgentFn.mockImplementation(
        async (_workDir, _featureId, prompt) => {
    ...
    it('should pass stage-specific model and options to runAgentFn', async () => {
      ...
      const stages = [
        { ...createMockStage('plan', 'Planning'), model: 'opus', max_turns: 20 },
        { ...createMockStage('implement', 'Implementation'), model: 'sonnet', max_turns: 10 },
      ];
    ...
    it('should pass accumulated context from stage N as previous_context to compileStage for stage N+1', async () => {
      ...
      mockSecureFs.readFile.mockImplementation((filePath: string) => {
        if (filePath.endsWith('agent-output.md')) {
          ...
          if (readCount === 2) return Promise.resolve('Plan output from stage 1');
          if (readCount === 3) return Promise.resolve('Plan output from stage 1\nImplement output from stage 2');
    ...
    it('should pass accumulated context as previousContent in runAgentFn options', async () => {
      ...
      mockSecureFs.readFile.mockImplementation((filePath: string) => {
        if (filePath.endsWith('agent-output.md')) {
          ...
          return Promise.resolve(`Context after stage ${agentOutputReadCount - 1}`);
        }
        return Promise.reject(new Error('ENOENT'));
      });
