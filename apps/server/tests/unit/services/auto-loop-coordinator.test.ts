...
describe("auto-loop-coordinator.ts", () => {
  ...
  describe("startAutoLoopForProject", () => {
    it("throws if loop already running for project/worktree", async () => {
      ...
      await expect(
        coordinator.startAutoLoopForProject("/test/project", null, 1),
      ).rejects.toThrow(
    ...
    it("creates ProjectAutoLoopState with correct config", async () => {
      await coordinator.startAutoLoopForProject(
        "/test/project",
        "feature-branch",
        2,
      );

      const config = coordinator.getAutoLoopConfigForProject(
        "/test/project",
        "feature-branch",
      );
    ...
    it("calls saveExecutionState", async () => {
      ...
      expect(mockSaveExecutionState).toHaveBeenCalledWith(
        "/test/project",
        null,
        3,
      );
    ...
    it("uses settings maxConcurrency when not provided", async () => {
      const result = await coordinator.startAutoLoopForProject(
        "/test/project",
        null,
      );
  ...
  describe("stopAutoLoopForProject", () => {
    it("aborts running loop", async () => {
      ...
      const result = await coordinator.stopAutoLoopForProject(
        "/test/project",
        null,
      );
      ...
      expect(
        coordinator.isAutoLoopRunningForProject("/test/project", null),
      ).toBe(false);
    });
    ...
    it("calls clearExecutionState", async () => {
      ...
      expect(mockClearExecutionState).toHaveBeenCalledWith(
        "/test/project",
        null,
      );
    });
  ...
  describe("isAutoLoopRunningForProject", () => {
    ...
    it("returns false for different worktree", async () => {
      ...
      expect(
        coordinator.isAutoLoopRunningForProject("/test/project", "branch-b"),
      ).toBe(false);
    });
  });
  ...
  describe("runAutoLoopForProject", () => {
    ...
    it("executes features within concurrency limit", async () => {
      ...
      vi.mocked(
        mockConcurrencyManager.getRunningCountForWorktree,
      ).mockResolvedValue(0);
    ...
    it("waits when at capacity", async () => {
      ...
      vi.mocked(
        mockConcurrencyManager.getRunningCountForWorktree,
      ).mockResolvedValue(2); // At capacity for maxConcurrency=2
    ...
    it("counts all running features (auto + manual) against concurrency limit", async () => {
      ...
      expect(
        mockConcurrencyManager.getRunningCountForWorktree,
      ).toHaveBeenCalledWith("/test/project", null, undefined);
    });
  ...
  describe("failure tracking", () => {
    it("trackFailureAndCheckPauseForProject returns true after threshold", async () => {
      ...
      const result1 = coordinator.trackFailureAndCheckPauseForProject(
        "/test/project",
        {
          type: "agent_error",
          message: "Error 1",
      ...
      const result2 = coordinator.trackFailureAndCheckPauseForProject(
        "/test/project",
        {
          type: "agent_error",
          message: "Error 2",
      ...
      const result3 = coordinator.trackFailureAndCheckPauseForProject(
        "/test/project",
        {
          type: "agent_error",
          message: "Error 3",
    ...
    it("agent errors count as failures", async () => {
      ...
      const result = coordinator.trackFailureAndCheckPauseForProject(
        "/test/project",
        {
          type: "agent_error",
  ...
  describe("multiple projects", () => {
    it("runs concurrent loops for different projects", async () => {
      ...
      expect(coordinator.isAutoLoopRunningForProject("/project-a", null)).toBe(
        true,
      );
      expect(coordinator.isAutoLoopRunningForProject("/project-b", null)).toBe(
        true,
      );
    ...
    it("runs concurrent loops for different worktrees of same project", async () => {
      ...
      expect(
        coordinator.isAutoLoopRunningForProject(
          "/test/project",
          "feature-branch",
        ),
      ).toBe(true);
      ...
      await coordinator.stopAutoLoopForProject("/test/project", null);
      await coordinator.stopAutoLoopForProject(
        "/test/project",
        "feature-branch",
      );
    });
  ...
  describe("getRunningCountForWorktree", () => {
    it("delegates to ConcurrencyManager", async () => {
      ...
      const count = await coordinator.getRunningCountForWorktree(
        "/test/project",
        null,
      );
    ...
    it("passes autoModeOnly option to ConcurrencyManager", async () => {
      ...
      expect(
        mockConcurrencyManager.getRunningCountForWorktree,
      ).toHaveBeenCalledWith("/test/project", null, { autoModeOnly: true });
    });
  });
  ...
  describe("edge cases", () => {
    it("handles null settingsService gracefully", async () => {
      ...
      const result = await coordWithoutSettings.startAutoLoopForProject(
        "/test/project",
        null,
      );
    ...
    it("handles resetStuckFeatures error gracefully", async () => {
      vi.mocked(mockResetStuckFeatures).mockRejectedValue(
    ...
    it("bypasses dependency checks when loadAllFeaturesFn is omitted", async () => {
      ...
      vi.mocked(
        mockIsFeatureRunning as ReturnType<typeof vi.fn>,
      ).mockImplementation((id: string) => id === "dep-feature");

      await coordWithoutLoadAll.startAutoLoopForProject(
        "/test/project",
        null,
        1,
      );
  ...
  describe("auto_mode_idle emission timing (idle check fix)", () => {
    ...
    it("handles loadAllFeaturesFn error gracefully (falls back to emitting idle)", async () => {
      vi.mocked(mockLoadPendingFeatures).mockResolvedValue([]);
      vi.mocked(mockLoadAllFeatures).mockRejectedValue(
        new Error("Failed to load features"),
      );
      vi.mocked(
        mockConcurrencyManager.getRunningCountForWorktree,
      ).mockResolvedValue(0);
