...
vi.mock('@pegasus/utils', () => ({
  ...
  isValidBranchName: (name: string) => /^[a-zA-Z0-9._/-]+$/.test(name) && !name.startsWith('-'),
...
describe('merge-service', () => {
  ...
  describe('standard merge (non-squash)', () => {
    it('performs a regular merge and returns success', async () => {
      ...
      expect(mockEmitter.emit).toHaveBeenCalledWith('merge:start', expect.any(Object));
      expect(mockEmitter.emit).toHaveBeenCalledWith('merge:success', expect.any(Object));
    });
  ...
  describe('conflict detection', () => {
    it('detects conflicts from merge output', async () => {
      ...
      expect(mockEmitter.emit).toHaveBeenCalledWith('merge:conflict', expect.any(Object));
    });
  });
  ...
  describe('deleteWorktreeAndBranch', () => {
    it('deletes worktree and branch when option is set', async () => {
      ...
      const branchCalls = mockExecGitCommand.mock.calls.filter(
        (call: unknown[]) =>
          Array.isArray(call[0]) && call[0][0] === 'branch' && call[0][1] === '-D'
      );
