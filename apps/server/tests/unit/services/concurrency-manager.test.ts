...
describe("ConcurrencyManager", () => {
  ...
  describe("getRunningCountForWorktree", () => {
    it("should return 0 when no features are running", async () => {
      const count = await manager.getRunningCountForWorktree(
        "/test/project",
        null,
      );
      expect(count).toBe(0);
    ...
    it("should count features with exact branch match for feature worktrees", async () => {
      ...
      const mainWorktreeCount = await manager.getRunningCountForWorktree(
        "/test/project",
        null,
      );
    ...
    it("should respect branch normalization (main is treated as null)", async () => {
      ...
      const mainCount = await manager.getRunningCountForWorktree(
        "/test/project",
        null,
      );
    ...
    it("should count only auto-mode features on specific worktree when autoModeOnly is true", async () => {
      ...
      manager.updateRunningFeature("feature-auto", {
        branchName: "feature-branch",
      });
      ...
      manager.updateRunningFeature("feature-manual", {
        branchName: "feature-branch",
      });
      ...
      manager.updateRunningFeature("feature-other", {
    ...
    it("should filter by both projectPath and branchName", async () => {
      ...
      const countA = await manager.getRunningCountForWorktree(
        "/project-a",
        "feature-x",
      );
      const countB = await manager.getRunningCountForWorktree(
        "/project-b",
        "feature-x",
      );
  ...
  describe("edge cases", () => {
    ...
    it("should handle features across different worktrees", async () => {
      ...
      expect(
        await manager.getRunningCountForWorktree("/test/project", null),
      ).toBe(1);
      expect(
        await manager.getRunningCountForWorktree("/test/project", "branch-a"),
      ).toBe(1);
      expect(
        await manager.getRunningCountForWorktree("/test/project", "branch-b"),
      ).toBe(1);
      expect(manager.getRunningCount("/test/project")).toBe(3);
    });
