...
export class IdeationService {
  ...
  async sendMessage(
    ...
    try {
      ...
      const existingWorkContext =
        await this.gatherExistingWorkContext(projectPath);
  ...
  async analyzeProject(projectPath: string): Promise<ProjectAnalysisResult> {
    ...
    try {
      ...
      const suggestions = await this.generateAnalysisSuggestions(
        projectPath,
        structure,
      );
  ...
  async generateSuggestions(
    ...
    try {
      ...
      const existingWorkContext = await this.gatherExistingWorkContext(
        projectPath,
        {
...
${contextSection}${existingWorkSection}`;
  ...
  private parseSuggestionsFromResponse(
    ...
    try {
      ...
      if (!jsonMatch) {
        logger.warn(
          "No JSON array found in response, falling back to text parsing",
        );
      ...
      if (titleMatch) {
        ...
        if (currentSuggestion && currentSuggestion.title) {
          suggestions.push({
            ...
            description:
              currentContent.join(" ").trim() || currentSuggestion.title,
            rationale: "",
            priority: "medium",
            ...currentSuggestion,
   ...
   */
  private async gatherBasicProjectInfo(
    projectPath: string,
  ): Promise<string | null> {
    const parts: string[] = [];
    ...
    try {
      ...
      const keyDeps = depNames
        .filter(
          (d) =>
            !d.startsWith("@types/") &&
            !["typescript", "eslint", "prettier"].includes(d),
        )
        .slice(0, 15);
      if (keyDeps.length > 0) {
    ...
    if (cachedAnalysis) {
      ...
      if (cachedAnalysis.routes && cachedAnalysis.routes.length > 0) {
        parts.push(
          `**Routes:** ${cachedAnalysis.routes.map((r) => r.name).join(", ")}`,
        );
      }
      ...
      try {
        ...
        if (features.length > 0) {
          ...
          if (byStatus["in-progress"].length > 0) {
            parts.push(
              `**In Progress:** ${byStatus["in-progress"].join(", ")}`,
            );
          }
  ...
  private async generateAnalysisSuggestions(
    ...
    if (structure.routes.length > 0 && structure.routes.length < 5) {
      suggestions.push({
        ...
        description:
          "The app has a small number of routes. Consider adding more features.",
    ...
    if (
      ...
      suggestions.push({
        ...
        description:
          "Consider adding React Query or similar for better data management.",
        rationale:
  ...
  private generateAnalysisSummary(
    ...
    if (structure.framework) {
      parts.push(
        `${structure.framework} ${structure.language || ""} application`,
      );
    }
