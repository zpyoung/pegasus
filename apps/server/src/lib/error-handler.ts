...
const ERROR_PATTERNS: ErrorPattern[] = [
  ...
  // Rate limit errors
  {
    ...
    suggestedAction:
      "Wait a few minutes before retrying, or consider upgrading your plan.",
    retryable: true,
  },
  ...
  // Timeout errors
  {
    ...
    suggestedAction:
      "Try again with a simpler request or check your connection.",
    retryable: true,
  },
  ...
  // CLI not found
  {
    ...
    suggestedAction:
      "Please install the required CLI tool and ensure it's in your PATH.",
    retryable: false,
  },
...
export function logError(
  ...
  logger.error(
    `Error in ${provider || "unknown"}${operation ? ` during ${operation}` : ""}`,
    {
...
export const ProviderErrorHandler = {
  claude: {
    ...
    getUserMessage: (error: unknown) =>
      getUserFriendlyErrorMessage(error, "claude"),
  ...
  codex: {
    ...
    getUserMessage: (error: unknown) =>
      getUserFriendlyErrorMessage(error, "codex"),
    isAuth: (error: unknown) => isAuthenticationError(error),
    isBilling: (error: unknown) => isBillingError(error),
    isRateLimit: (error: unknown) => isRateLimitError(error),
  },
  ...
  cursor: {
    ...
    getUserMessage: (error: unknown) =>
      getUserFriendlyErrorMessage(error, "cursor"),
    isAuth: (error: unknown) => isAuthenticationError(error),
    isBilling: (error: unknown) => isBillingError(error),
    isRateLimit: (error: unknown) => isRateLimitError(error),
  },
