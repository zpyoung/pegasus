...
export function createResolvePRCommentHandler() {
  return async (req: Request, res: Response): Promise<void> => {
    try {
      const { projectPath, threadId, resolve } =
        req.body as ResolvePRCommentRequest;
      ...
      if (typeof resolve !== "boolean") {
        res
          .status(400)
          .json({ success: false, error: "resolve must be a boolean" });
        return;
      }
      ...
      const result = await executeReviewThreadMutation(
        projectPath,
        threadId,
        resolve,
      );
