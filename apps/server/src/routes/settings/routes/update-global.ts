...
export function createUpdateGlobalHandler(settingsService: SettingsService) {
  return async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const projectsLen = Array.isArray(updates.projects)
        ...
        for (const project of projects) {
          try {
            ...
            if (terminalConfigEnabled) {
              ...
              logger.info(
                `[TERMINAL_CONFIG] Regenerated RC files for project: ${project.name}`,
              );
            }
          } catch (error) {
            logger.warn(
      ...
      if (
        "enableRequestLogging" in updates &&
        typeof updates.enableRequestLogging === "boolean"
      ) {
