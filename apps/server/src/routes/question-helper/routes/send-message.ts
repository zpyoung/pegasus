...
function parseModelEntry(
  ...
  if (entry.thinkingLevel !== undefined && typeof entry.thinkingLevel !== 'string') {
    return { ok: false, error: 'modelEntry.thinkingLevel must be a string when provided' };
  }
  if (entry.providerId !== undefined && typeof entry.providerId !== 'string') {
    return { ok: false, error: 'modelEntry.providerId must be a string when provided' };
  }
...
export function createSendMessageHandler(helperService: QuestionHelperService) {
  return async (req: Request, res: Response): Promise<void> => {
    ...
    helperService
      .sendMessage(featureId, message.trim(), projectPath, modelEntryResult.value)
      .catch((err: unknown) => {
