...
export async function syncSpec(
  ...
  logger.info(
    `Current spec has ${currentImplementedFeatures.length} implemented features`,
  );
  ...
  logger.info(
    `Found ${completedFeatures.length} completed/verified features in Pegasus`,
  );
  ...
  const existingNames = new Set(
    currentImplementedFeatures.map((f) => f.name.toLowerCase()),
  );
...
3. No explanations, no markdown, no text before or after the JSON.`;
  ...
  try {
    ...
    if (techResult.structured_output) {
      ...
      const structured =
        techResult.structured_output as unknown as TechStackExtractionResult;
      ...
      const extracted = extractJson<TechStackExtractionResult>(
        techResult.text,
        {
          logger,
  ...
  for (const phase of currentRoadmapPhases) {
    ...
    if (relatedCompletedFeatures.length > 0 && phase.status !== "completed") {
      ...
      specContent = updateRoadmapPhaseStatus(
        specContent,
        phase.name,
        newStatus,
      );
  ...
  if (result.techStackUpdates.added.length > 0) {
    summaryParts.push(
      `Added ${result.techStackUpdates.added.length} technologies`,
    );
  }
  if (result.techStackUpdates.removed.length > 0) {
    summaryParts.push(
      `Removed ${result.techStackUpdates.removed.length} technologies`,
    );
  }
