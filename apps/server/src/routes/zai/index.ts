...
export function createZaiRoutes(
  ...
  (async () => {
    try {
      ...
      logger.error(
        "[init] Failed to load z.ai API key from credentials:",
        error,
      );
    }
  })();
    ...
    try {
      ...
      if (!isAvailable) {
        ...
        res.status(200).json({
          ...
          message:
            "Set Z_AI_API_KEY environment variable to enable z.ai usage tracking",
        });
        return;
      }
  ...
  router.post("/configure", async (req: Request, res: Response) => {
    try {
        ...
        try {
          ...
          if (
            ...
            res.status(400).json({
              success: false,
              error:
                "Invalid request: apiHost must be a valid HTTP or HTTPS URL",
            });
            return;
          }
        } catch {
      ...
      const sanitizedHost =
        typeof apiHost === "string" ? apiHost.trim() : undefined;
  ...
  router.get("/status", async (_req: Request, res: Response) => {
    try {
      ...
      res.json({
        ...
        hasEnvApiKey,
        message: isAvailable
          ? "z.ai API is configured"
          : "z.ai API token not configured",
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : "Unknown error";
