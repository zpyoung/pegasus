...
describe("resolver.ts", () => {
  describe("resolveDependencies", () => {
    ...
    it("should use default priority 2 when not specified", () => {
      ...
      const ids = result.orderedFeatures.map((f) => f.id);
      expect(ids.indexOf("HighPriority")).toBeLessThan(
        ids.indexOf("NoPriority"),
      );
      expect(ids.indexOf("NoPriority")).toBeLessThan(
        ids.indexOf("LowPriority"),
      );
    });
  ...
  describe("wouldCreateCircularDependency", () => {
    ...
    it("should return false for valid linear dependency chain", () => {
      ...
      expect(wouldCreateCircularDependency(featuresWithD, "C", "D")).toBe(
        false,
      );
    });
    ...
    it("should handle feature not in list", () => {
      ...
      expect(wouldCreateCircularDependency(features, "NonExistent", "A")).toBe(
        false,
      );

      // Non-existent target - should return false
      expect(wouldCreateCircularDependency(features, "A", "NonExistent")).toBe(
        false,
      );
    });
    ...
    it("should handle longer transitive chains", () => {
      ...
      expect(wouldCreateCircularDependency(featuresWithF, "E", "F")).toBe(
        false,
      );
    });
    ...
    it("should handle diamond dependency pattern", () => {
      ...
      expect(wouldCreateCircularDependency(featuresWithE, "D", "E")).toBe(
        false,
      );
    });
  });
