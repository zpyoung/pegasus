...
describe("error-handler.ts", () => {
  ...
  describe("isQuotaExhaustedError", () => {
    it("should return true for overloaded errors", () => {
      expect(
        isQuotaExhaustedError(new Error("overloaded_error: service is busy")),
      ).toBe(true);
      expect(isQuotaExhaustedError(new Error("Server is overloaded"))).toBe(
        true,
      );
    ...
    it("should return true for usage limit errors", () => {
      ...
      expect(isQuotaExhaustedError(new Error("Usage limit exceeded"))).toBe(
        true,
      );
      ...
      expect(isQuotaExhaustedError(new Error("session limit reached"))).toBe(
        true,
      );
      expect(isQuotaExhaustedError(new Error("weekly limit hit"))).toBe(true);
      expect(isQuotaExhaustedError(new Error("monthly limit reached"))).toBe(
        true,
      );
    });
    ...
    it("should return true for billing/credit errors", () => {
      expect(
        isQuotaExhaustedError(new Error("credit balance is too low")),
      ).toBe(true);
      expect(isQuotaExhaustedError(new Error("insufficient credits"))).toBe(
        true,
      );
      expect(isQuotaExhaustedError(new Error("insufficient balance"))).toBe(
        true,
      );
      expect(isQuotaExhaustedError(new Error("no credits remaining"))).toBe(
        true,
      );
      expect(isQuotaExhaustedError(new Error("out of credits"))).toBe(true);
      expect(isQuotaExhaustedError(new Error("billing issue detected"))).toBe(
        true,
      );
    ...
    it("should return true for upgrade prompts", () => {
      expect(
        isQuotaExhaustedError(new Error("Please /upgrade your plan")),
      ).toBe(true);
      expect(isQuotaExhaustedError(new Error("extra-usage not enabled"))).toBe(
        true,
      );
    });

    it("should return false for regular errors", () => {
      expect(isQuotaExhaustedError(new Error("Something went wrong"))).toBe(
        false,
      );
  ...
  describe("isModelNotFoundError", () => {
    ...
    it("should return true for model_not_found errors", () => {
      expect(
        isModelNotFoundError(new Error("model_not_found: gpt-5.3-codex")),
      ).toBe(true);
    });

    it("should return true for invalid_model errors", () => {
      expect(
        isModelNotFoundError(new Error("invalid_model: unknown model")),
      ).toBe(true);
    });

    it('should return true for "model does not exist" errors', () => {
      expect(isModelNotFoundError(new Error("The model does not exist"))).toBe(
        true,
      );
    });
    ...
    it("should return false for regular errors", () => {
      expect(isModelNotFoundError(new Error("Something went wrong"))).toBe(
        false,
      );
  ...
  describe("isStreamDisconnectedError", () => {
    ...
    it('should return true for "stream ended" errors', () => {
      expect(
        isStreamDisconnectedError(new Error("stream ended unexpectedly")),
      ).toBe(true);
    });

    it('should return true for "connection reset" errors', () => {
      expect(
        isStreamDisconnectedError(new Error("connection reset by peer")),
      ).toBe(true);
    });
    ...
    it("should return false for regular errors", () => {
      expect(isStreamDisconnectedError(new Error("Something went wrong"))).toBe(
        false,
      );
