...
export function validateSpec(
  spec: SpecOutput | null | undefined,
): ValidationResult {
  const errors: string[] = [];
  ...
  if (!Array.isArray(spec.technology_stack)) {
    ...
    spec.technology_stack.some((t) => typeof t !== "string" || t.trim() === "")
  ) {
  ...
  if (!Array.isArray(spec.core_capabilities)) {
    ...
    spec.core_capabilities.some((c) => typeof c !== "string" || c.trim() === "")
  ) {
  ...
  if (!Array.isArray(spec.implemented_features)) {
    ...
    spec.implemented_features.forEach((f, i) => {
      if (!f.name || typeof f.name !== "string" || f.name.trim() === "") {
        errors.push(
          `implemented_features[${i}].name is required and must be a non-empty string`,
        );
      }
      if (!f.description || typeof f.description !== "string") {
        errors.push(
          `implemented_features[${i}].description is required and must be a string`,
        );
      }
      if (f.file_locations !== undefined) {
        if (!Array.isArray(f.file_locations)) {
          errors.push(
            `implemented_features[${i}].file_locations must be an array if provided`,
          );
        } else if (
          f.file_locations.some(
            (loc) => typeof loc !== "string" || loc.trim() === "",
          )
        ) {
          errors.push(
            `implemented_features[${i}].file_locations items must be non-empty strings`,
          );
        }
      }
    });
  ...
  if (spec.additional_requirements !== undefined) {
    if (!Array.isArray(spec.additional_requirements)) {
      ...
      spec.additional_requirements.some(
  ...
  if (spec.development_guidelines !== undefined) {
    if (!Array.isArray(spec.development_guidelines)) {
      ...
      spec.development_guidelines.some(
    ...
    if (!Array.isArray(spec.implementation_roadmap)) {
      ...
      spec.implementation_roadmap.forEach((r, i) => {
        ...
        if (!r.description || typeof r.description !== "string") {
          errors.push(
            `implementation_roadmap[${i}].description is required and must be a string`,
          );
        }
      });
    }
...
export function isValidSpecXml(xmlContent: string): boolean {
  ...
  const hasProjectName = /<project_name>[\s\S]*?<\/project_name>/.test(
    xmlContent,
  );
  const hasOverview = /<overview>[\s\S]*?<\/overview>/.test(xmlContent);
  const hasTechStack = /<technology_stack>[\s\S]*?<\/technology_stack>/.test(
    xmlContent,
  );
  const hasCapabilities =
    /<core_capabilities>[\s\S]*?<\/core_capabilities>/.test(xmlContent);

  return (
    hasRoot && hasProjectName && hasOverview && hasTechStack && hasCapabilities
  );
}
